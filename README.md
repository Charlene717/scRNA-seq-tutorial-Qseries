...
88 題單選（Q1 20 題、Q2 30 題、Q3 38 題），中英對照，含影片內自測題與常見錯誤、腳本練習的延伸題。互動題庫在 [`quiz/`](quiz/)：把資料夾下載到電腦後，點擊 `index.html` 即可開啟作答——即點即答、附解析、可切換中英文。
...
每份都分「精選」（日常分析從這裡選就夠）與「延伸」（什麼情況值得跳出精選），並附決策樹與「做到哪一段該翻哪幾個資源」的對應表。
