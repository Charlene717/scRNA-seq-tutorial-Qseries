...
> **最新更新 — v1.2.1（2026-09-13）**：修正數處教材文字與程式／數學不一致的地方（QC 的方法段模板、稀疏度判讀、LISI 的上限、`requireNamespace()` 的說明），ORA 輸出補上探索性標記，執行時間全面改用實測值，進階練習的階段 D 改成「七個步驟都要評估、適用的才跑」。上一版 v1.2.0 是 00–10 全流程實跑後的同步修正，並撤回了 v1.1.0 在 `07` 加的細胞標籤改名。用舊版做過相關章節的話，請看[版本更新紀錄](CHANGELOG.md)。
