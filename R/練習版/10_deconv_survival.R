...
#       所以就算換一個全新的專案資料夾，也是直接命中舊檔案。其餘同一台機器三輪實測 3–4 分鐘。
# ⚠ 這支有兩個地方會「不是你的錯」卻跑不完，兩個都是 GDC 伺服器當下不通：GDCquery（查有哪些檔）
#   與 GDCprepare（下載完之後再連一次 API 補臨床欄位）。兩處都會自動重試三次；
#   真的連不上就過幾分鐘再跑，已經下載好的檔案完全不受影響，不必重下載。
...
# GDCquery 只是去問「這個專案有哪些檔」，一個網路請求而已，但 GDC 的 API 三不五時會不通，
# 這時 TCGAbiolinks 會直接丟 "GDC server down, try to use this package later"。
# 那是對方的狀態，不是你的程式或資料有問題——所以跟下面的 GDCprepare 一樣重試三次再放棄。
q <- NULL
for (try.i in 1:3) {
  q <- tryCatch(GDCquery(project = "TCGA-GBM", data.category = "Transcriptome Profiling",
                         data.type = "Gene Expression Quantification", workflow.type = "STAR - Counts"),
                error = function(e) { message("  GDCquery 第 ", try.i, " 次失敗：",
                                              sub("\n.*", "", conditionMessage(e))); NULL })
  if (!is.null(q)) break
  if (try.i < 3) { message("  20 秒後重試……"); Sys.sleep(20) }
}
if (is.null(q))
  stop("GDCquery 連續 3 次失敗，多半是 GDC 伺服器當下不通（訊息通常是 GDC server down）。\n",
       "  這一步只是去問「有哪些檔」，還沒開始下載，所以你本機的東西一個都沒動到。\n",
       "  過幾分鐘從本行重跑即可；要確認對方是不是在維護：https://portal.gdc.cancer.gov/ \n",
       "  趕時間的話，本節的結論不影響 00–09 的任何結果，可以先跳過 10。", call. = FALSE)
