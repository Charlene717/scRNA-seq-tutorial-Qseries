...
cumu <- cumsum(pct)
# 下面兩個門檻是經驗法則，不是定律：請對照 ElbowPlot 一起看，數字不合理就自己調。
...
#   cluster  8：mt = 1.4（最低）、cluster 1：mt = 1.7（次低）、cluster 10：nF = 878（最低）
#               → 分別是免疫、寡樹突與 T 細胞的典型輪廓
#   其餘十群 dbl ≤ 0.02（13 群裡只有 12、9、7 三群超過 0.02）。
...
# 判讀：
#   ② nCount 接近「兩個來源群相加」＋ ③ 同一顆細胞同時亮互斥譜系 ＋ ④ 沒有專屬 marker
#     → 真 doublet，03 整群移除並記錄。
#   ④ 有明確的專屬 marker、且 ② 對不上任何一組來源群相加 → 被工具誤判的真實族群，不要刪。
#   ⚠ 不要用「是不是單一群的兩倍」當標準：只有同型 doublet（同一種細胞相撞）才會翻倍。
#     異型 doublet 要拿兩個親代群的中位數相加來比，數字通常遠低於任一群的兩倍。
...
## ---- 4c. 什麼時候刪、刪到什麼程度 ----------------------------------- Q2 頁 36
# 「要不要刪 doublet」沒有單一答案，看三件事。這一節不產生輸出，是決策用的對照表。
#
# 一、平台決定了 doublet 有多嚴重
#   液滴式（10x、Drop-seq）：兩顆細胞進同一個液滴，率隨上樣量升高，1,000 顆約 0.8%、
#     10,000 顆可到 8%。這種資料一定要處理。本課 Q2 用的就是 10x。
#   孔盤式（Smart-seq2、CEL-seq2）：一孔一顆是設計出來的，doublet 率低很多、成因也不同
#     （分選失誤而非隨機共封裝）。Q3 的 GSE84465 屬於這一類，所以 04–10 沒有再跑一次 doublet 偵測。
#   換資料前先問平台是什麼，不要把 10x 的做法直接搬到孔盤資料上。
#
# 二、腫瘤資料的誤判方向是固定的，所以「先刪」有風險
#   doublet 工具靠的是「轉錄體看起來像兩顆細胞混在一起」，而 RNA 量大、轉錄體複雜的細胞
#   天生就長那樣——惡性細胞與增殖細胞正是這種。所以在腫瘤資料裡，誤判會系統性地集中在
#   你最不想弄丟的族群身上（本課 cluster 7 增殖群拿到 0.20、cluster 9 基質群 0.31 就是例子）。
#   在還沒判定惡性之前就大刀砍下去，很可能砍掉的是惡性細胞本身。
#   反過來說，CNV 證據到手之後，判斷會容易得多：
#     - 候選群有乾淨、且與某位病人一致的 CNV 圖譜 → 是惡性細胞，不是 doublet
#     - 候選群的 CNV 呈現「介於參考與惡性之間」的中間帶，又同時亮互斥譜系 → 才是真 doublet
#   所以本課的順序是：01 只標記 → 02 用群層級三項診斷 → 03 只移除確認的
#   → 05 拿到 CNV 之後，如果還有存疑的群，可以回頭再判一次。
#
# 三、下游分析對 doublet 的容忍度差很多，決定你要刪得多乾淨
#   分析                     敏感度   為什麼
#   ---------------------------------------------------------------------
#   inferCNV 的參考組         很高     參考組混進 doublet，等於參考本身帶了 CNV 訊號，
#                                      整張熱圖的振幅會被壓低，惡性與正常的界線跟著糊掉。
#                                      參考組要用最保守的標準挑（見 05 §1a）。
#   細胞通訊（CellChat）      很高     一顆 doublet 同時帶著配體與受體，會憑空生出
#                                      不存在的通訊對，而且常常出現在「新發現」的位置。
#   軌跡（Slingshot）         很高     doublet 落在兩群中間，正好被連成一條假的過渡路徑
#                                      或假分支——這是最容易產生假故事的一種。
#   分群與註釋                中       整群 doublet 會冒充成「新的中間型別」。
#   組成分析                  中       比例被稀釋或灌水，尤其當 doublet 集中在某幾個樣本。
#   pseudobulk 差異表達       低       彙總到病人 × 型別之後，少數 doublet 影響很小。
#
#   實務上的建議：要做通訊或軌跡之前，把候選群一起排除（或至少各跑一次比較結果差多少）；
