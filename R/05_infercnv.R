...
#   純 CNV 版（否決之前）：整體一致 87.4%｜判為惡性中相符 98.1%（629/641）｜抓回 59.1%（629/1,064）
#   最終版（CNV＋譜系否決）：整體一致 87.6%｜判為惡性中相符 99.4%（629/633）｜抓回 59.1%（629/1,064）
#   ⚠ 兩者之差就是譜系否決替這個數字加的分：相符率 98.1% → 99.4%（+1.3 個百分點），
#     而抓回率完全不動（629/1,064 兩版一樣）——否決只把錯的拿掉，不會多抓對的回來。
#   ⚠⚠ 但純 CNV 版也不是獨立驗證：它只移除了循環的第 ② 項（譜系否決），
#     第 ① 項還在——inferCNV 的參考組本來就是用 celltype_author 挑的。
#     真正獨立的標準答案要來自完全沒參與分類的證據（配對基因體 CNV、突變、另一份標註）。
