...
#
## >>> 參考答案 ------------------------------------------------------
# 本課的結果（實跑驗證，加權平均病人熵。數學上限是四位病人等量時的 log(4) = 1.386；
# 另一個常用的參考點是「鄰域完全照全域病人比例混合」時的 -Σp·lnp = 1.24
# （本資料 488/1,157/1,511/383）——那是參考值，不是上限）：
...
# LISI 的數學上限就是病人數 4（鄰域裡四位病人各佔四分之一時達到，Q3 頁 16 算過）；
#   熵的上限同理是 log(4) = 1.386。這兩個數字跟資料無關，永遠是這樣。
# ⚠ 但「四位病人各佔四分之一」要求每一個鄰域都等量，而本資料四位病人的細胞數是
#   488 / 1,157 / 1,511 / 383，差到四倍。若每個鄰域都剛好反映全域比例，算出來是
#   1/Σp² = 3.13、熵 -Σp·lnp = 1.24。這是「照現有組成充分混合」的參考值，不是上限——
#   局部仍然可以超過它，全體平均卻很難。而且各細胞型別自己的病人組成不一樣，
#   要嚴謹比較的話，參考值該用那個型別自己的組成重算，不是照抄全域的 3.13。
# 所以要看的是「相對關係」，不是「離某個數字多近」：
...
#
## >>> 參考答案 ------------------------------------------------------
# 本課的結果（各型別的 LISI 中位數，實跑驗證。上限是病人數 4；照全域組成充分混合的參考值 3.13）：
...
#   整合後惡性的熵 1.09，已經是「照全域組成充分混合」那個參考值 1.24 的 88% —— 拿參考值來看，
#   「被混掉」比對著數學上限 1.386 看更明顯。
