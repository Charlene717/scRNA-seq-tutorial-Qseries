...
> 上表的時間是 2026-09-12 在 Windows、R 4.4.1、Seurat 5.3.0 從乾淨環境跑完 00–10 的實測值（12 支共約 34 分鐘）。`05` 隨機器與細胞數差異大，`10` 的 TCGA 下載目錄在專案之外，換一個新專案資料夾也不會重新下載。每張圖都會在 `output/figs/` 存成 png 與 pdf 兩份（同名不同副檔名）；
