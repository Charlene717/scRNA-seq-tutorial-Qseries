...
props <- getTransformedProps(clusters = gbm4$type, sample = sample.id, transform = "logit")
...
pb_de <- function(obj, type, min.cells = MIN_CELLS, min.pairs = MIN_PAIRS) {
  ...
  dds <- DESeqDataSetFromMatrix(pb[rowSums(pb) >= 10, ], coldata, design = ~ patient + tissue)
...
volcano <- function(d, fc = 1, p = 0.05) {
  ...
  EnhancedVolcano(r, lab = r$gene, x = "log2FC", y = "padj",
...
run_gsea <- function(d, sets, minSize = 15, maxSize = 500) {
  ...
  out <- lapply(names(sets), function(nm) {
    g <- fgsea(sets[[nm]], r, minSize = minSize, maxSize = maxSize, nproc = 1)   # eps 留預設
...
## >>> 參考答案 ------------------------------------------------------
# 本課這份資料的結果（實跑驗證；FOCUS 自動選到 Immune cell）：
#   de.summary：Immune cell，4 對病人，padj < 0.05 共 608 個基因。
#     細胞數 ≥ 100 的四種型別裡，Malignant / Neoplastic / OPC 都因配對數不足被跳過（見 §2b）。
#   假重複的代價：cell-level Wilcoxon 判出 3,486 個顯著基因，pseudobulk 只有 608——差 5.7 倍。
#     多出來的不是新發現，是把「同一位病人的幾百顆細胞」當成幾百個獨立樣本換來的假陽性。
#   Hallmark GSEA（正 NES = 核心較高）：E2F_TARGETS 2.12、EMT 2.05、GLYCOLYSIS 1.92、
#     G2M_CHECKPOINT 1.89、OXIDATIVE_PHOSPHORYLATION 1.86。
#   HYPOXIA 的 leading edge：SLC2A1、TGM2、CA12、AK4、GPC4、ANGPTL4、RRAGD、LDHA…
#     全是教科書級的缺氧基因，方向也對（核心缺氧）——已知答案驗證通過，可以往下讀。
#
# ★ 但這份結果有一個一定要講的限制，它正好打中 §2 開頭那句話。
#   看 head(foc.res, 15) 排最前面的基因：TPSAB1、TPSB2、CTSG、HDC——這四個全是「肥大細胞」的標誌基因，
#   而且 log2FC 高達 21–23。這麼大的倍數幾乎只有一種成因：一邊幾乎是 0，另一邊才有。
#   換句話說，這個「差異」很可能不是免疫細胞的表現變了，而是核心多了一群邊緣沒有的肥大細胞——
#   是「Immune cell」這個桶子內部的組成改變。
#   原因在於我們用的是作者的粗標籤：巨噬細胞、小膠質、T 細胞、肥大細胞全被歸成同一個 Immune cell。
#   §2 開頭說「把所有細胞混在一起比，得到的是組成差異」，這裡是同一個問題縮小一級後再度出現。
#   正確的做法是先把免疫細胞分出亞群（像 03 §4b 做的），再對每個亞群各自跑 pseudobulk。
#   本課保留這個結果，是因為它是最好的示範：分得夠不夠細，決定你比到的是表現還是組成。
#
# 另外兩個讀結果時要留意的地方：
#   ① log2FC 超過 ±10 的基因（這裡的 LOC100507206 −26.6、LOC387895 −26.1）幾乎都是「一邊全 0」造成的，
#      不是效應真的那麼大。要看 baseMean 與每樣本點圖確認，別直接寫進結論。
#   ② GO_BP 有 7,583 個基因集，跑出 METANEPHRIC_NEPHRON_DEVELOPMENT（腎臟）、RESPONSE_TO_CADMIUM_ION
#      這種明顯不相干的詞條是常態——基因集之間共用基因，數量一多就會撈到雜訊。
#      GO_BP 當探索用，寫結論要回到 Hallmark 這種經過整理、規模小而明確的集合。
## <<< 參考答案
...
#      在動手前先寫下：這份資料符合「細胞通訊」那條路的資料前提嗎？（Q3 頁 59）
# =====================================================================
