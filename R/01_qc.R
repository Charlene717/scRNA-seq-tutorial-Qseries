...
nf.lo <- 10^mad.lo(log10(gbm$nFeature_RNA))           # 這兩個指標通常右偏；取 log10 後較接近對稱，MAD 才站得住
...
#       不要為了「讓門檻有作用」改成 nc.hi <- quantile(gbm$nCount_RNA, 0.995)——那條線天生就砍掉
#       最高的 0.5%，被砍的不一定有品質問題。固定百分位要用的話，是另做一次敏感度分析來對照，
#       不是拿來取代 MAD 這一條。超高深度多半是 doublet，交給 §4 的 DoubletFinder 判斷更準。
#       方法段照樣要寫這一條，並註明它移除 0 顆：事先訂好又實際套用的規則，就算沒砍到人也要報，
#       別人才照得出你的流程。
## <<< 參考答案
...
# 「什麼時候刪、刪到什麼程度」要看平台、看有沒有先判惡性、看下游分析多敏感——
# 三者的對照表在 02_cluster.R §4c。腫瘤資料尤其不要在判定惡性之前就大刀砍。
